...
export default function InputJurnalPage() {
  ...
  if (!cutoff) {
    return (
      <div className="flex-1 flex items-center justify-center p-6 bg-slate-50/40 h-full overflow-auto">
        <div className="bg-white rounded-xl border border-slate-200 max-w-sm w-full overflow-hidden shadow-md">
          {/* Flat White Header with Thin grey bottom line */}
          <div className="p-6 border-b border-slate-200/80 bg-slate-50/50 text-center">
            <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-lg flex items-center justify-center mx-auto mb-4 border border-emerald-100">
              <FileCheck className="w-6 h-6" />
            </div>
            <h2 className="text-lg font-bold text-slate-900 tracking-tight">Buka Bulan Buku</h2>
            <p className="text-slate-500 text-xs font-semibold mt-1">Pilih unit kebun dan periode sebelum menginput.</p>
          </div>
          
          <form onSubmit={handleCutoffSubmit} className="p-6 space-y-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                <Building2 className="w-3.5 h-3.5 text-slate-400" /> Unit Kebun
              </label>
              <select 
                required
                className="w-full border border-slate-200 bg-white rounded-md px-3 py-1.5 text-xs font-semibold text-slate-700 focus:ring-1 focus:ring-emerald-500 outline-none transition-all cursor-pointer h-9"
                value={tempUnit}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  type="number" 
                  required
                  className="w-full border border-slate-200 bg-white rounded-md px-3 py-1.5 text-xs font-semibold text-slate-700 focus:ring-1 focus:ring-emerald-500 outline-none transition-all h-9"
                  value={tempTahun}
  ...
  return (
    <div className="flex-1 overflow-auto bg-slate-50/40 p-6 lg:p-8 space-y-6 flex flex-col">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/80 pb-5">
        ...
        {/* Active Session Info */}
        <div className="bg-white border border-slate-200 rounded-xl px-4 py-2 flex items-center gap-3.5 shadow-sm">
          <div className="flex items-center gap-2">
            <Clock className="w-3.5 h-3.5 text-slate-400" />
            <div className="text-xs font-bold text-slate-800">
              Sesi: <span className="text-emerald-700">Unit {cutoff.unit} ({BULAN_OPTIONS.find(b=>b.value===cutoff.bulan)?.label} {cutoff.tahun})</span>
            </div>
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col flex-1">
        <div className="px-5 py-3 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          ...
          <div className="hidden sm:flex items-center gap-1.5 bg-white px-2.5 py-1 rounded-md border border-slate-200 shadow-sm text-[9px] font-bold text-slate-400 uppercase tracking-wider">
        ...
        <form onSubmit={handleSaveJurnal} className="p-5 flex-1 flex flex-col justify-between">
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 gap-4 bg-slate-50 p-4 rounded-lg border border-slate-200/80">
              <div className="md:col-span-2 space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                   <Calendar className="w-3.5 h-3.5 text-slate-400" /> Tanggal Bukti
                </label>
                <input 
                  type="date" 
                  required
                  className="w-full bg-white border border-slate-200 rounded-md px-3 py-1.5 text-xs font-semibold text-slate-700 focus:ring-1 focus:ring-emerald-500 outline-none transition-all h-9"
              ...
              <div className="md:col-span-4 space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between">
                   <div className="flex items-center gap-1.5">
                      <Hash className="w-3.5 h-3.5 text-slate-400" /> Nomor Bukti Transaksi
                   </div>
                   <Button 
                    type="button"
                    variant="ghost"
                ...
                <input 
                  ...
                  className="w-full bg-white border border-slate-200 rounded-md px-3 py-1.5 text-xs font-bold text-slate-800 focus:ring-1 focus:ring-emerald-500 outline-none transition-all h-9"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider flex items-center justify-between px-1">
                  Rekening Debit
                  <span className="bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase border border-emerald-100">MASUK (+)</span>
                </label>
                <CustomCombobox 
                  items={rekening} 
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between px-1">
                  Rekening Kredit (Lawan)
                  <span className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase border border-slate-200">KELUAR (-)</span>
                </label>
                <CustomCombobox 
                  items={rekening} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
              <div className="md:col-span-4 space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider px-1">Uraian Transaksi / Deskripsi</label>
                <input 
                  type="text" 
                  maxLength={255}
                  className="w-full border border-slate-200 rounded-md px-3 py-1.5 text-xs font-semibold text-slate-700 focus:ring-1 focus:ring-emerald-500 outline-none transition-all h-9"
              ...
              <div className="md:col-span-2 space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider px-1">Nilai Nominal Jurnal</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    min="0"
                    step="0.01"
                    className="w-full border border-slate-200 rounded-md pl-8 pr-3 py-1.5 text-xs font-bold text-slate-800 focus:ring-1 focus:ring-emerald-500 outline-none transition-all font-mono h-9"
                    placeholder="0.00"
          ...
          <div className="mt-8 pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-end gap-3">
            <Button 
              ...
              className="w-full sm:w-auto px-4 h-9 bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 rounded-md font-bold text-xs transition-all"
            >
              Reset Form
            </Button>
            <Button 
