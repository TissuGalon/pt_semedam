...
export function PayrollSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const user = {
    name: "HRD Semedam",
    email: "hrd@semedam.co.id",
    avatar: ""
  };

  const navItems = [
    { name: 'Dashboard HR & Gaji', href: '/portal/payroll', icon: Home },
    { name: 'Rekap Buruh Panen (BHL)', href: '#', icon: Users2 },
    { name: 'Kalkulasi Borongan TBS', href: '#', icon: Sparkles },
    { name: 'Slip Gaji Bulanan', href: '#', icon: FileText },
    { name: 'Kalender Rekap Afdeling', href: '#', icon: Calendar },
  ];
    ...
    <Sidebar collapsible="icon" className="border-r border-slate-200 dark:border-zinc-800 bg-white dark:bg-sidebar text-slate-900 dark:text-zinc-100" {...props}>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <div className="flex items-center gap-3.5 w-full overflow-hidden">
              <div className="w-8.5 h-8.5 rounded-lg bg-emerald-600 flex items-center justify-center text-white font-extrabold text-lg shrink-0 shadow-sm">
                S
              </div>
              <div className={cn("transition-all duration-200 flex flex-col min-w-0", isCollapsed ? "opacity-0 w-0" : "opacity-100")}>
                ...
                <span className="text-xs font-black text-slate-900 dark:text-zinc-100 tracking-wider mt-1.5 uppercase">HRD & Payroll</span>
              </div>
            </div>
          </SidebarMenuItem>
        </SidebarMenu>
      ...
      <SidebarContent className="px-2 py-3 space-y-4">
        ...
        <SidebarGroup className="p-0">
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton asChild className="text-emerald-650 hover:text-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 font-black text-[10px] uppercase tracking-widest gap-2">
                <Link href="/portal">
        ...
        {/* Menu Utama */}
        <SidebarGroup className="p-0">
          <SidebarGroupLabel className="text-[9px] font-black text-slate-400 dark:text-zinc-555 uppercase tracking-widest px-2.5 mb-1.5">Menu Payroll</SidebarGroupLabel>
          <SidebarMenu>
            {navItems.map((item) => {
              ...
              return (
                <SidebarMenuItem key={item.name}>
                  <SidebarMenuButton asChild isActive={isActive} className={cn("text-xs font-bold gap-3 rounded-xl py-2 px-2.5 transition-colors cursor-pointer", isActive ? "bg-slate-100 dark:bg-zinc-800 text-emerald-600 dark:text-emerald-400 font-extrabold" : "text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-900/50 hover:text-slate-900 dark:hover:text-zinc-200")}>
                    <Link href={item.href}>
                      <Icon className={cn("w-4 h-4 shrink-0", isActive ? "text-emerald-600" : "text-slate-455 dark:text-zinc-550")} />
                      <span>{item.name}</span>
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              ...
              <DropdownMenuTrigger asChild>
                <SidebarMenuButton size="lg" className="hover:bg-slate-100 dark:hover:bg-zinc-850 rounded-xl cursor-pointer">
                  <Avatar className="h-7 w-7 rounded-lg shrink-0">
                    <AvatarFallback className="bg-emerald-100 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-450 text-xs font-black">HS</AvatarFallback>
                  </Avatar>
