...
export function InventorySidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const user = {
    name: "Logistik Semedam",
    email: "inventory@semedam.co.id",
    avatar: ""
  };

  const navItems = [
    { name: 'Dashboard Logistik', href: '/portal/inventory', icon: Home },
    { name: 'Mutasi Masuk Gudang', href: '#', icon: Warehouse },
    { name: 'Pengeluaran Barang', href: '#', icon: ClipboardList },
    { name: 'Stok Minimum Alert', href: '#', icon: AlertTriangle },
    { name: 'Log Append Gudang', href: '#', icon: Database },
  ];
    ...
    <Sidebar collapsible="icon" className="border-r border-slate-200 dark:border-zinc-800 bg-white dark:bg-sidebar text-slate-900 dark:text-zinc-100" {...props}>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <div className="flex items-center gap-3.5 w-full overflow-hidden">
              <div className="w-8.5 h-8.5 rounded-lg bg-blue-600 flex items-center justify-center text-white font-extrabold text-lg shrink-0 shadow-sm">
                S
              </div>
              <div className={cn("transition-all duration-200 flex flex-col min-w-0", isCollapsed ? "opacity-0 w-0" : "opacity-100")}>
                ...
                <span className="text-xs font-black text-slate-900 dark:text-zinc-100 tracking-wider mt-1.5 uppercase">Inventory Gudang</span>
              </div>
            </div>
          </SidebarMenuItem>
        </SidebarMenu>
      ...
      <SidebarContent className="px-2 py-3 space-y-4">
        ...
        <SidebarGroup className="p-0">
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton asChild className="text-blue-600 hover:text-blue-750 hover:bg-blue-50 dark:hover:bg-blue-950/20 font-black text-[10px] uppercase tracking-widest gap-2">
                <Link href="/portal">
        ...
        {/* Menu Utama */}
        <SidebarGroup className="p-0">
          <SidebarGroupLabel className="text-[9px] font-black text-slate-400 dark:text-zinc-550 uppercase tracking-widest px-2.5 mb-1.5">Menu Logistik</SidebarGroupLabel>
          <SidebarMenu>
            {navItems.map((item) => {
              ...
              return (
                <SidebarMenuItem key={item.name}>
                  <SidebarMenuButton asChild isActive={isActive} className={cn("text-xs font-bold gap-3 rounded-xl py-2 px-2.5 transition-colors cursor-pointer", isActive ? "bg-slate-100 dark:bg-zinc-800 text-blue-600 dark:text-blue-400 font-extrabold" : "text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-900/50 hover:text-slate-900 dark:hover:text-zinc-200")}>
                    <Link href={item.href}>
                      <Icon className={cn("w-4 h-4 shrink-0", isActive ? "text-blue-600" : "text-slate-400 dark:text-zinc-550")} />
                      <span>{item.name}</span>
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              ...
              <DropdownMenuTrigger asChild>
                <SidebarMenuButton size="lg" className="hover:bg-slate-100 dark:hover:bg-zinc-850 rounded-xl cursor-pointer">
                  <Avatar className="h-7 w-7 rounded-lg shrink-0">
                    <AvatarFallback className="bg-blue-100 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 text-xs font-black">LS</AvatarFallback>
                  </Avatar>
